  ...
  return <>
    ...
    <section className="panel mb-6">
      ...
      <form action={createClassificationAction} className="form-grid">
        ...
        <label className="field">Grupo DRE<select name="dreGroup"><option value="VARIABLE_COST_EXPENSE">Custos e Despesas Variáveis</option><option value="FIXED_COST_EXPENSE">Custos e Despesas Fixas</option></select></label>
    ...
    <section className="grid gap-4">
      {classifications.map((item) => <article className="panel" key={item.id}>
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div><h2 className="section-title">{item.name}</h2><p className="mt-1 text-sm text-slate-500">{item.code} · {groupLabel[item.dreGroup]} · {item._count.accountsPayable} conta(s)</p></div>
        ...
        <div className="mt-4 grid gap-4 md:grid-cols-2">
          <details><summary className="link-button">Editar</summary><form action={updateClassificationAction} className="mt-3 grid gap-2"><input name="id" type="hidden" value={item.id} /><label className="field">Código técnico<input disabled value={item.code} /></label><label className="field">Nome<input defaultValue={item.name} maxLength={200} name="name" required /></label><label className="field">Grupo DRE<select defaultValue={item.dreGroup} name="dreGroup" disabled={item._count.accountsPayable > 0}><option value="VARIABLE_COST_EXPENSE">Custos e Despesas Variáveis</option><option value="FIXED_COST_EXPENSE">Custos e Despesas Fixas</option></select>{item._count.accountsPayable > 0 ? <><input name="dreGroup" type="hidden" value={item.dreGroup} /><small>Grupo bloqueado porque a classificação já está em uso.</small></> : null}</label><label className="field">Observações<input defaultValue={item.notes ?? ""} maxLength={2000} name="notes" /></label><SubmitButton>Salvar</SubmitButton></form></details>
