...
export default async function Page({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ success?: string; error?: string }> }) {
  ...
  const [account, messages] = await Promise.all([
    prisma.accountPayable.findUnique({ where: { id }, include: { company: true, classification: true, payments: { orderBy: [{ paymentDate: "desc" }, { createdAt: "desc" }] }, contractorSettlement: { include: { contractor: true } } } }),
    searchParams,
  ]);
  if (!account) notFound();
  ...
  return <>
    ...
    <section className="panel mb-5"><dl className="detail-grid">
      ...
      <div><dt>Grupo DRE snapshot</dt><dd>{groupLabel[account.dreGroupSnapshot]}</dd></div>
