...
export default async function Page({ searchParams }: { searchParams: Promise<Params> }) {
  ...
  return <>
    ...
    <section className="panel"><div className="table-wrap"><table><thead><tr><th>Empresa</th><th>Vencimento</th><th>Descrição</th><th>Origem</th><th>Beneficiário</th><th>Classificação</th><th>Impacta DRE</th><th>Grupo DRE</th><th>Competência</th><th>Original</th><th>Pago</th><th>Saldo</th><th>Situação</th></tr></thead><tbody>{filtered.map((row) => <tr key={row.id}><td>{row.company.tradeName || row.company.name}</td><td>{formatDate(row.dueDate)}</td><td><Link className="link-button" href={`/financeiro/contas-a-pagar/${row.id}`}>{row.description}</Link></td><td>{row.source === "MANUAL" ? "Lançamento manual" : "Fechamento"}</td><td>{row.payeeName}</td><td>{row.classificationNameSnapshot}</td><td>{row.financialNatureSnapshot === "OPERATING_EXPENSE" ? "Sim" : "Não"}</td><td>{row.dreGroupSnapshot ? groupLabel[row.dreGroupSnapshot] : "Não aplicável"}</td><td>{formatDate(row.competenceDate)}</td><td>{formatCurrency(row.originalAmount)}</td><td>{formatCurrency(paidAmount(row.payments))}</td><td>{formatCurrency(remainingAmount(row.originalAmount, row.payments))}</td><td>{financialStatus(row.originalAmount, row.dueDate, row.payments, today)}</td></tr>)}</tbody></table></div></section>
  </>;
}
