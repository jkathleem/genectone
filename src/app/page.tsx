...
const shortcuts = [["Nova OP", "/ops/nova"], ["Consultar OPs", "/ops"], ["Empresas", "/cadastros/empresas"], ["Clientes", "/cadastros/clientes"], ["Produtos", "/cadastros/produtos"], ["Terceirizados", "/cadastros/terceirizados"]];

export default async function Home() {
  const values = await Promise.all([prisma.productionOrder.count(), prisma.company.count(), prisma.customer.count(), prisma.product.count(), prisma.contractor.count()]);
  const counts = ["OPs", "Empresas", "Clientes", "Produtos", "Terceirizados"].map((label, index) => [label, values[index]]);
