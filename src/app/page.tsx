...
const shortcuts = [["Nova OP", "/ops/nova"], ["Consultar OPs", "/ops"], ["Empresas", "/cadastros/empresas"], ["Clientes", "/cadastros/clientes"], ["Produtos", "/cadastros/produtos"], ["Terceirizados", "/cadastros/terceirizados"], ["Serviços", "/cadastros/servicos"]];

export default async function Home() {
  const values = await Promise.all([prisma.productionOrder.count(), prisma.company.count(), prisma.customer.count(), prisma.product.count(), prisma.contractor.count(), prisma.outsourcedService.count()]);
  const counts = ["OPs", "Empresas", "Clientes", "Produtos", "Terceirizados", "Serviços terceirizados"].map((label, index) => [label, values[index]]);
