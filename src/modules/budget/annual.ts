...
export async function annualBudgetComparison(companyId:string,year:number){
 ...
 const budgetStatuses=Array.from({length:12},(_,month)=>budgets.find(x=>x.competenceDate.getUTCMonth()===month)?.status??null);
 return {lines,details,budgetStatuses,margins:{plannedContribution:annualMargin(plannedTotals.contributionMargin,plannedTotals.grossRevenue),actualContribution:annualMargin(actualTotals.contributionMargin,actualTotals.grossRevenue),plannedOperating:annualMargin(plannedTotals.operatingProfit,plannedTotals.grossRevenue),actualOperating:annualMargin(actualTotals.operatingProfit,actualTotals.grossRevenue),plannedNet:annualMargin(plannedTotals.managerialNetIncome,plannedTotals.grossRevenue),actualNet:annualMargin(actualTotals.managerialNetIncome,actualTotals.grossRevenue)}};
}
