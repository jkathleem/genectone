...
export async function cashFlowData({ companyId, from, to, view }: Params) {
  if (view === "actual") {
    const [receipts, payments] = await Promise.all([
      ...
      prisma.payment.findMany({ where: { paymentDate: { gte: from, lte: to }, accountPayable: { companyId: companyId || undefined } }, include: { accountPayable: { include: { company: true } } }, orderBy: { paymentDate: "asc" } }),
    ]);
    const movements: CashMovement[] = [...receipts.map(item => ({ id: item.id, date: item.receiptDate, direction: "IN" as const, type: "Recebimento", company: item.company.tradeName || item.company.name, description: `Recebimento — ${item.customer.name} — ${item._count.allocations} conta(s) alocada(s)`, counterparty: item.customer.name, amount: item.amount, href: `/financeiro/recebimentos/${item.id}` })), ...payments.map(item => ({ id: item.id, date: item.paymentDate, direction: "OUT" as const, type: "Pagamento", company: item.accountPayable.company.tradeName || item.accountPayable.company.name, description: item.accountPayable.description, counterparty: item.accountPayable.payeeName, amount: item.amount, href: `/financeiro/contas-a-pagar/${item.accountPayableId}` }))];
  ...
  const payableInclude = { payments: { select: { amount: true } }, company: true } as const;
  ...
  for (const item of payables) { const paid = sumBalances(item.payments.map(x => x.amount)), amount = positiveBalance(item.originalAmount, paid); if (amount.gt(0)) movements.push({ id: item.id, date: item.dueDate, direction: "OUT", type: "Conta a Pagar", company: item.company.tradeName || item.company.name, description: item.description, counterparty: item.payeeName, amount, originalAmount: item.originalAmount, settledAmount: paid, href: `/financeiro/contas-a-pagar/${item.id}` }); }
