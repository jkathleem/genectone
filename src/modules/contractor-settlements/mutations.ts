...
export async function updateDraft(db: Database, id: string, data: { periodMonth: number; periodYear: number; notes: string | null; contractorId: string; companyId: string }) {
  return db.$transaction(async (tx) => {
    ...
    if (settlement.companyId !== data.companyId && settlement._count.items) throw new Error("Remova os itens antes de alterar a empresa.");
...
export async function addDraftItem(db: Database, settlementId: string, outsourcedServiceId: string, quantity: number) {
  ...
  return db.$transaction(async (tx) => {
    ...
    if (!settlement) throw new Error("Fechamento não encontrado.");
    assertDraft(settlement.status);
    const service = await tx.outsourcedService.findUnique({ where: { id: outsourcedServiceId }, include: { productionOrder: { select: { companyId: true } }, settlementItems: { include: { settlement: true } } } });
    ...
    if (service.productionOrder.companyId !== settlement.companyId) throw new Error("Este serviço pertence a outra empresa e não pode ser incluído neste fechamento.");
