...
export async function addServicePrice(data: FormData) { await run("Novo preço registrado.", async () => { const unitPrice = parseMoneyInput(String(data.get("unitPrice"))); if (unitPrice.lt(0)) throw new Error("negative"); return prisma.servicePrice.create({ data: { serviceId: z.string().cuid().parse(data.get("id")), unitPrice, validFrom: new Date(`${z.string().regex(/^\d{4}-\d{2}-\d{2}$/, "Informe a data de vigência.").parse(data.get("validFrom"))}T00:00:00.000Z`) } }); }); }
