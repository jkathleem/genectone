...
export async function registerPayment(formData: FormData) {
  ...
  try {
    ...
    await createPayment(prisma, id, { paymentDate: new Date(`${date}T00:00:00.000Z`), amount: parseMoneyInput(String(formData.get("amount") ?? "")), notes: String(formData.get("notes") ?? "") });
