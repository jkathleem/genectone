...
type PaymentValue = { amount: Prisma.Decimal | string };
export function competenceDate(year: number, month: number) { return new Date(`${year}-${String(month).padStart(2, "0")}-01T00:00:00.000Z`); }
export function paidAmount(payments: PaymentValue[] = []) { return payments.reduce((sum, payment) => sum.plus(payment.amount), new Prisma.Decimal(0)); }
