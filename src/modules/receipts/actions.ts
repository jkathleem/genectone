...
export async function registerReceipt(formData: FormData) {
  ...
  try {
    ...
    const receipt = await createReceipt(prisma, { companyId, customerId, receiptDate: new Date(`${date}T00:00:00.000Z`), amount: parseMoneyInput(String(formData.get("amount") ?? "")), notes: String(formData.get("notes") ?? ""), allocations });
